function Level(plan) {
  ...
  this.width = plan[0].length; // Begins at the X axis.
  // Use the number of rows to set the height

  this.height = plan.length; // Begins at the Y axis.
...
function Vector(x, y) {
  this.x = x; this.y = y;  // The "x" & "y" values here are from the Counters above.
}

// Vector arithmetic: v_1 + v_2 = <a,b>+<c,d> = <a+c,b+d>
Vector.prototype.plus = function(other) {   // "other" represents the other Vector.
...
function Player(pos) {
  this.pos = pos.plus(new Vector(0, -0.5)); // "-0.5" makes sure that the player doesn't spawn in the ground, since they're 50% taller than the base.
...
Level.prototype.obstacleAt = function (pos, size) {
  var xStart = Math.floor(pos.x); // Rounds a decimal down. I.E. 4.999999 to 4.
  var xEnd = Math.ceil(pos.x + size.x); // Rounds a decimal up. I.E. 4.99999 to 5.
  var yStart = Math.floor(pos.y);
  var yEnd = Math.ceil(pos.y + size.y);

  // Consider the sides, top, & bottom of the level as walls.
  if (xStart < 0 || xEnd > this.width || yStart < 0 || yEnd > this.height)
    return "wall";

    // Check each grid position starting at yStart, x starting
    // checking for possible obstacles (non null value)
...
Player.prototype.moveX = function(step, level, keys) {
  this.speed.x = 0; // "0" indicates the Player's speed, i.e. 0 at not moving.
  if (keys.left) this.speed.x -= playerXSpeed;
  if (keys.right) this.speed.x += playerXSpeed;

  var motion = new Vector(this.speed.x * step, 0); //Takes the base speed of "0", i.e. not moving, then multiplies it by the constant "7" or "-7" speed set on playerXSpeed.
  var newPos = this.pos.plus(motion);

  // Find if there's an obstacle at the newPos
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  var resetPos = new Vector(10,10); // Respawn position.
  ...
  if (obstacle) {
    if (keys.up && this.speed.y > 0) // If the player is moving downward I.E. falling.
